...
def _use_case(fake_repo, gas_repo, parser, archiver, logger, clock):
    return ArchiveImportUseCase(fake_repo, gas_repo, parser, archiver, logger, clock)


def test_import_at_any_date(fake_repo, gas_repo, parser, archiver, logger, clock):
    path = Path("data_2020-05-05.csv")
    parser.add_csv(path, date(2020, 5, 5), Decimal("123.45"))
    result = _use_case(fake_repo, gas_repo, parser, archiver, logger, clock).run([path])
...
def test_corrupt_file_stays_in_place(fake_repo, gas_repo, parser, archiver, logger, clock):
    path = Path("data_2020-05-05.csv")
...
def test_already_archived_file_not_moved_again(fake_repo, gas_repo, parser, archiver, logger, clock):
    path = Path("data_2020-05-05.csv")
...
def test_manual_day_backfill_preserves_override(fake_repo, gas_repo, parser, archiver, logger, clock):
    day = date(2020, 5, 5)
    fake_repo.save_manual(day, Decimal("50"))
    path = Path("data_2020-05-05.csv")
    parser.add_csv(path, day, Decimal("10"))
    _use_case(fake_repo, gas_repo, parser, archiver, logger, clock).run([path])
